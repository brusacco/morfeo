...
namespace :entries do
    ...
    Entry.enabled.where(published_at: DAYS_RANGE.days.ago..Time.current).find_in_batches(batch_size: 500) do |batch|
      batch.each do |entry|
        ...
        begin
          ...
          next if title_tags.empty?

          matching_topics = Topic.joins(:tags).where(tags: { name: title_tags }).distinct
  ...
  task :check_topic_sync, [:topic_id] => :environment do |_t, args|
    ...
    synced_entries = topic.entries.enabled.where(published_at: DAYS_RANGE.days.ago..Time.current).count
