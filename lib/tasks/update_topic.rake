...
module TopicUpdateCacheInvalidation
  module_function

  def service_cache_patterns(topic_id)
    [
      "topic_#{topic_id}_*",
...
namespace :topic do
  ...
  task :update, %i[topic_id days] => :environment do |_t, args|
    # ============================================
    ...
    puts "\n⚠️  WARNING: Topic has no tags. Some operations will be skipped." if tag_names.empty?
    ...
    service_cache_patterns = TopicUpdateCacheInvalidation.service_cache_patterns(topic.id)
    ...
    if entries_total > 0
      entries_scope.find_each.with_index do |entry, index|
        ...
        # Progress indicator
        puts "   ⏳ Progress: #{index + 1}/#{entries_total} entries..." if (index + 1) % 50 == 0
    ...
    if fb_total > 0
      fb_scope.find_each.with_index do |facebook_entry, index|
        ...
        # Progress indicator
        puts "   ⏳ Progress: #{index + 1}/#{fb_total} Facebook entries..." if (index + 1) % 50 == 0
      rescue StandardError => e
    ...
    if tw_total > 0
      tw_scope.find_each.with_index do |twitter_post, index|
        ...
        # Progress indicator
        puts "   ⏳ Progress: #{index + 1}/#{tw_total} Twitter posts..." if (index + 1) % 50 == 0
      rescue StandardError => e
    ...
    entries_with_tags = Entry.enabled.where(published_at: days.days.ago..Time.current).tagged_with(tag_names, any: true)

    synced_count = 0

    entries_with_tags.find_each do |entry|
      matching_topics = Topic.joins(:tags).where(tags: { name: entry.tag_list }).distinct
    ...
    entries_with_title_tags.find_each do |entry|
      matching_topics = Topic.joins(:tags).where(tags: { name: entry.title_tag_list }).distinct
    ...
    date_range.each do |day_date|
      ActiveRecord::Base.transaction do
        ...
        title_stat.update!(entry_quantity: title_entry_quantity, entry_interaction: title_entry_interaction)
      end
  ...
  task :update_multiple, %i[topic_ids days] => :environment do |_t, args|
