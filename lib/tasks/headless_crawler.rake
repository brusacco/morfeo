...
namespace :crawler do
  desc 'Scrape JavaScript-rendered news sites using Chrome Headless'
  ...
  task :site, [:site_ids] => :environment do |_t, args|
    ...
    if site_ids.empty?
      ...
      puts "Usage: rake crawler:headless:site[1,2,3]"
      exit 1
    end

    puts "\n🚀 Starting Headless Crawler for specific sites..."
    puts "Sites: #{site_ids.join(', ')}"
    puts "=" * 80

    result = HeadlessCrawlerServices::Orchestrator.call(site_ids: site_ids)
  ...
  task :test, [:limit] => :environment do |_t, args|
    ...
    puts "\n🧪 Testing Headless Crawler with #{limit} site(s)..."
    puts "=" * 80
...
desc 'Scrape JavaScript-rendered news sites (alias for crawler:headless)'
task headless_crawler: 'crawler:headless'
