...
task telegram_bot: :environment do
  ...
  def split_text(text)
    if text.length > 4000
      ...
      second_part = text[split_index..-1]
  ...
  Telegram::Bot::Client.run(TELEGRAM_TOKEN) do |bot|
    bot.listen do |message|
      ...
      when Telegram::Bot::Types::Message
        case message.text
        when '/start'
          bot.api.send_message(chat_id: message.chat.id, text: "Hola, #{message.from.first_name} bienvenido a MoopioBot.")
        ...
        when '/populares'
          ...
          bot.api.send_message(chat_id: message.chat.id, text: "A continuacion, mostramos las 10 noticias mas importantes")
          entries.each do |entry|
        ...
        when '/recientes'
          ...
          bot.api.send_message(chat_id: message.chat.id, text: "A continuacion, mostramos las 10 noticias mas recientes")
          entries.each do |entry|
            bot.api.send_message(chat_id: message.chat.id, text: "#{entry.title} #{entry.url}")
          end
        when /^\/tema (.+)/
          ...
          if topic.nil? || entries.empty?
            ...
            bot.api.send_message(chat_id: message.chat.id, text: "A continuacion, mostramos los 10 noticias mas importantes en el tema #{topic.name}")
            entries.each do |entry|
              bot.api.send_message(chat_id: message.chat.id, text: "#{entry.title} - Interacciones: #{entry.total_count} #{entry.url}")
            end
          end
        when /^\/etiqueta (.+)/
          etiqueta = message&.text.gsub('/etiqueta ', '')
          entries = Entry.includes(:site).a_week_ago.tagged_with(etiqueta, any: true).order(total_count: :desc).limit(10)

          if entries.empty?
            bot.api.send_message(chat_id: message.chat.id, text: "No se encontro este tema")
          else
            bot.api.send_message(chat_id: message.chat.id, text: "A continuacion, mostramos los 10 noticias mas importantes en el tema #{etiqueta}")
            entries.each do |entry|
              bot.api.send_message(chat_id: message.chat.id, text: "#{entry.title} - Interacciones: #{entry.total_count} #{entry.url}")
            end
          end
        when '/temas'
          @topics = Topic.all
          bot.api.send_message(chat_id: message.chat.id, text: "A continuacion, mostramos los topicos: #{@topics.pluck(:name).join(',')}")
        when '/topicos'
          @topics = Topic.all
          ...
          bot.api.send_message(
            ...
            reply_markup: {
              inline_keyboard: button_options.each_slice(2).to_a
            }.to_json
          )
        end
      when Telegram::Bot::Types::CallbackQuery 
        ...
        when /^\/etiqueta (.+)/
          ...
          if entries.empty?
            ...
            bot.api.send_message(chat_id: message.from.id, text: "A continuacion, mostramos los 10 noticias mas importantes en el tema #{selected_option}")
            entries.each do |entry|
        ...
        when /^\/topico (.+)/
          ...
          if topic.nil? || entries.empty?
            ...
            bot.api.send_message(chat_id: message.from.id, text: "A continuacion, mostramos los 10 noticias mas importantes en el tema #{topic.name}")
            entries.each do |entry|
              bot.api.send_message(chat_id: message.from.id, text: "#{entry.title} - Interacciones: #{entry.total_count} #{entry.url}")
            end
