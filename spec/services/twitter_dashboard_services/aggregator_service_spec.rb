...
RSpec.describe TwitterDashboardServices::AggregatorService do
  ...
  it 'reuses a cached snapshot while applying each requested top-post limit' do
    ...
    posts = double('posts')
    snapshot = { twitter_data: { total_posts: 3 } }
    cached_snapshots = {}
    ...
    [short_limit, large_limit].each do |service_instance|
      allow(service_instance).to receive(:twitter_posts).and_return(posts)
    end
    ...
    expect(short_limit.call[:twitter_data]).to include(posts: posts, top_posts: [:short_limit_post])
    expect(large_limit.call[:twitter_data]).to include(posts: posts, top_posts: [:large_limit_post])
  end
  ...
  it 'returns empty dashboard data when the topic has no tags' do
    ...
    expect(empty_service.send(:load_twitter_data)).to include(
  ...
  it 'falls back to safe temporal defaults when source methods fail' do
    ...
    %i[
      ...
      twitter_content_half_life
      twitter_peak_publishing_times_by_hour
      twitter_peak_publishing_times_by_day
      twitter_engagement_heatmap_data
    ].each do |method_name|
  ...
  it 'reuses memoized tag names and combined text analysis for the dashboard posts' do
    ...
    expect(TwitterPost).to receive(:for_topic)
      .with(topic,
            start_time: service.instance_variable_get(:@start_time),
            end_time: service.instance_variable_get(:@end_time),
            tag_ids: [1, 2])
      .and_return(posts)
    expect(TwitterPost).to receive(:text_occurrences).with(posts).and_return(text_analysis)

    result = service.send(:load_twitter_data)
