ActiveAdmin.register Newspaper do
  ...
  permit_params :date, :site_id, :cover, :backcover, newspaper_texts_attributes: [:id, :title, :description, :_destroy]
  form do |f|
    f.inputs 'Archivos' do
      f.input :site_id, label:'Medio', as: :select, collection: Site.all.map {|t| [t.name, t.id]}, input_html: { required: true }
