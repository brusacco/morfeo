...
.to_h
    ...
    @polarities_entries_percentages =
      polarities_entries_counts.transform_values do |count|
        (count / polarity_entries_total_count * 100).round(0)
      end

    polarities_interactions_counts = @entries.where.not(polarity: nil).group(:polarity).sum('total_count').sort_by do |key, _value|
      key
    end
.to_h
    polarities_interactions_total_count = polarities_interactions_counts.values.sum.to_f
    @polarities_interactions_percentages =
      polarities_interactions_counts.transform_values do |count|
    ...
    @ai_reports = topic.reports.where.not(report_text: nil).where.not(created_at: Time.zone.now.all_day).order(created_at: :desc).limit(10)
