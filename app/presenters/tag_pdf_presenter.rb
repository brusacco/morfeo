...
# Presenter for Tag PDF generation
# Encapsulates all view logic and calculations for tag PDF reports
# Similar to DigitalPdfPresenter but adapted for tags
class TagPdfPresenter
  include ActionView::Helpers::NumberHelper
  include PdfConstants

  attr_reader :tag, :days_range, :data
  ...
  # Initialize presenter with tag data
  #
  # @param data [Hash] Data from TagPdfServices::PdfService
  # @param tag [Tag] The tag being analyzed
  # @param days_range [Integer] Number of days in the report period
  def initialize(data:, tag: nil, days_range: nil)
    @data = data
    @tag = tag
  ...
  def entries_count
    @entries_count ||= data.dig(:tag_data, :entries_count) ||
                       data.dig(:tag_data, :total_entries) ||
                       0
  end
  ...
  def interactions_count
    @interactions_count ||= data.dig(:tag_data, :entries_total_sum) ||
                            data.dig(:tag_data, :total_interactions) ||
                            0
  end
  ...
  def positive_sentiment
    {
      ...
      interactions: data.dig(:tag_data, :entries_polarity_sums, 'positive') || 0
    }
  end
  ...
  def neutral_sentiment
    {
      ...
      interactions: data.dig(:tag_data, :entries_polarity_sums, 'neutral') || 0
    }
  end
  ...
  def negative_sentiment
    {
      ...
      interactions: data.dig(:tag_data, :entries_polarity_sums, 'negative') || 0
    }
  end
  ...
  def has_sentiment_data?
    data.dig(:tag_data, :entries_polarity_counts).present?
  end

  # Check if site data exists
  # @return [Boolean] True if site data present
  def has_site_data?
    data.dig(:tag_data, :site_counts).present? &&
      data.dig(:tag_data, :site_counts).any?
  end
